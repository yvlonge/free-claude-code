...
CAPABILITY_CONTRACTS: tuple[CapabilityContract, ...] = (
    ...
    CapabilityContract(
        "local_providers",
        "local_api_openai_chat",
        "local_api_endpoint",
        "providers.local_api.LocalAPIProvider",
        "Anthropic request converted to OpenAI chat body and local_api base URL",
        "Anthropic-compatible SSE stream via OpenAI transport",
        "SSE error event for local upstream failure",
    ...
    CapabilityContract(
        "provider_routing",
        "weighted_target_pool_routing",
        "weighted_target_pools",
        "providers.registry.ProviderTargetPool and api.services.ClaudeProxyService",
        "MODEL* weighted pool string and provider failures",
        "weighted round-robin healthy selection and pre-output failover",
        "retryable failures trigger cooldown; exhausted pool returns HTTP 503 with Retry-After",
        (
